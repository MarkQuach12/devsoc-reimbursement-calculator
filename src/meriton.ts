...
export function calculateReimbursement(group: meritonEnjoyer[], costOfMeriton: number, costOfDrinking: number): meritonEnjoyerOutput[] {
    if (group.length === 0) {
    ...
    for (const individual of group) {
        ...
        const reimbursementAmount = individual.isCancelled ? baseDeposit : baseDeposit + individual.refundAmount - costOfMeritonPerPerson - (individual.isDrinking ? costOfDrinkingPerPerson : 0);
