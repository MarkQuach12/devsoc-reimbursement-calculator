import { type meritonEnjoyer, calculateReimbursement } from "../src/meriton";
...
describe("calculateReimbursement — unit / integration cases (matches current implementation)", () => {
	test("basic error cases", () => {
		...
		expect(() => calculateReimbursement([], 1000, 500)).toThrow(
			"Group should not be empty"
		);
		expect(() => calculateReimbursement([user], -500, 500)).toThrow(
			"Cost of Meriton cannot be negative"
		);
		expect(() => calculateReimbursement([user], 1000, -500)).toThrow(
			"Cost of Drinking cannot be negative"
		);
	});
	...
	test("single drinker (not cancelled) — costOfDrinking deducted twice per current logic", () => {
		...
		expect(result[0]!.reimbursementAmount).toBe(
			50 - costOfMeriton - costOfDrinking
		);
	});
	...
	test("cancelled participants return base deposit (drinker/non-drinker)", () => {
		...
		const result = calculateReimbursement(
			[cancelledDrinker, cancelledNonDrinker],
			costOfMeriton,
			costOfDrinking
		);
	...
	test("mixed group allocation (one drinker, one non-drinker)", () => {
		...
		expect(rDrinker.reimbursementAmount).toBe(
			50 - costOfMeriton / 2 - costOfDrinking
		);
	...
	test("mixed group allocation (two drinker, two non-drinker) with refund", () => {
		...
		const result = calculateReimbursement(
			[drinker1, drinker2, nonDrinker1, nonDrinker2],
			costOfMeriton,
			costOfDrinking
		);
		...
		expect(rDrinker1.reimbursementAmount).toBe(
			50 - costOfMeriton / 4 - costOfDrinking / 2
		);
		expect(rDrinker2.reimbursementAmount).toBe(
			50 + 70 - costOfMeriton / 4 - costOfDrinking / 2
		);
